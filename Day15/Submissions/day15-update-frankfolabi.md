...
- **Task Completed:** Learnt how to use multiple providers to access other accounts in AWS organization and how to deploy k8s
- **Date and Time:** September 21 1605
...
## The training/webapp is deprecated at the time so I used docker/getting-started and expose port 80
...
output "service_endpoint" {
    value = try(
        "http://${local.status[0]["load_balancer"][0]["ingress"][0]["hostname"]}","(error passing hostname from status)"
    )
...
resource "aws_eks_cluster" "cluster" {
    ...
    # Ensure that IAM Role permissions are created and deleted after the EKS Cluster.
...
resource "aws_eks_node_group" "nodes" {
    ...
    # Ensure that IAM Role permissions are created and deleted after the EKS Node Group.
...
module "eks_cluster" {
    ...
    # t2.micre would not work because all the 4 ENIs would be used up and you wont be able to deploy your pods
